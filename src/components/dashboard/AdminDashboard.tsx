...
// ─── KPI Card ────────────────────────────────────────────────────────────────
function KpiCard({ title, value, change, changeType = "neutral", subText, icon: Icon, href, sparklineData, sparklineColor = "var(--color-primary)" }: KpiCardProps) {
  const rawId = useId();
  ...
  const content = (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 z-10">
        <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/80">{title}</span>
        <div className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-all duration-300 shadow-2xs", currentVariant.iconBg)}>
      ...
      {/* 4. Footer Subtext & Interactive Arrow */}
      <div className="flex items-center justify-between z-10 pt-1">
        {subText ? <p className="text-[11px] font-medium text-muted-foreground/90 truncate">{subText}</p> : <div />}

        {href && (
          <div className="flex items-center gap-0.5 text-xs font-semibold text-primary opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-0 -translate-x-1">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Badge variant="outline" className="h-8 gap-1.5 px-3 bg-card text-muted-foreground border-border self-start sm:self-auto">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          subText="Items below safety limit"
          icon={AlertTriangle}
        ...
        <KpiCard
          ...
          subText={`Inventory health: ${stockHealthPercentage}% safe`}
          icon={Bell}
          href="/alerts"
