...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const [productCount, orderCount] = await Promise.all([prisma.product.count({ where: { supplierId: id } }), prisma.order.count({ where: { supplierId: id } })]);

    if (productCount > 0 || orderCount > 0) {
      return NextResponse.json({ error: `Cannot delete supplier with ${productCount} products and ${orderCount} orders` }, { status: 409 });
    }
