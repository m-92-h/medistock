...
import {
  Plus, Search, AlertTriangle, Package, Edit, Eye, Trash2,
  Boxes, DollarSign, BarChart3, PieChart as PieIcon,
  FilterX, ChevronLeft, ChevronRight, ShieldCheck, XCircle, Loader2,
} from "lucide-react";
import {
...
interface Stats { totalProducts: number; totalLowStock: number; totalValue: number; }
...
export default function ProductsPage() {
  ...
  // ─── جلب الـ categories والـ suppliers مرة واحدة فقط ──────────────────────
  useEffect(() => {
  ...
  // ─── جلب المنتجات — الـ stats تأتي مدمجة في نفس الـ response ──────────────
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // FIX: استخراج الـ stats من نفس الـ response — لا يوجد /api/products/stats
  ...
  const handleDelete = async (product: Product) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // إعادة جلب للحصول على stats محدّثة مع الصفحة الحالية
        await fetchProducts();
      } else {
        const data = await res.json();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription className="text-xs text-muted-foreground mt-0.5">
                Total physical unit quantity stored across categories (current page)
              </CardDescription>
            </div>
          </CardHeader>
        ...
        <Card className="border border-border/70 bg-card shadow-xs flex flex-col justify-between">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs text-muted-foreground mt-0.5">
              Proportion of healthy vs. critical stock items (current page)
            </CardDescription>
          </CardHeader>
            ...
            {stockHealthData.length === 0 ? (
              ...
              <>
                <ResponsiveContainer width="100%" height={150}>
                  <PieChart>
                    <Pie
                      data={stockHealthData} dataKey="value" nameKey="name"
                      cx="50%" cy="50%" innerRadius={42} outerRadius={62}
                      paddingAngle={3} stroke="var(--color-card)" strokeWidth={2}
                    >
      ...
      <Card className="border border-border/70 bg-card shadow-xs">
        <CardContent className="p-4">
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
            ...
            <Select value={supplierId} onValueChange={(val) => { setSupplierId(val ?? "ALL"); setPage(1); }}>
              <SelectTrigger className="w-full lg:w-[180px] text-xs sm:text-sm bg-background">
                      ...
                      <TableCell className="text-center py-3 px-4">
                        <div className="flex justify-center items-center gap-1">
                          ...
                          {canDelete && (
                            <>
                              <Button
                                size="icon" variant="ghost" title="Edit Product"
                                className="h-8 w-8 cursor-pointer text-muted-foreground hover:text-foreground hover:bg-muted"
